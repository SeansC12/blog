...
function ID({ data }) {
  ...
  const [isThereUnsavedChanges, setIsThereUnsavedChanges] =
    useState(false);
    ...
    <div>
      ...
      <div className="flex justify-end items-center w-full sticky top-0">
        <div
          ...
          className="rounded-md bg-[#20ff63] py-[0.35rem] px-4 text-black font-lato w-max mr-5 hover:scale-105 transition-all ease-in cursor-pointer select-none"
        >
          Update
        </div>
        <div
          onClick={() => setIsDeleteMode(true)}
          className="rounded-md bg-[#fc4e4e] py-[0.35rem] px-4 text-black font-lato w-max mr-5 hover:scale-105 transition-all ease-in cursor-pointer select-none"
        >
          Delete
        </div>
      </div>
        ...
        {isEditingMode ? (
          <div className="w-full">
            ...
            <input
              ref={titleRef}
              onChange={() => {
                setBlogTitle(titleRef.current.value);
              }}
              defaultValue={blogTitle}
              className="border-2 rounded border-gray-500 w-full mb-10"
            />
...
export async function getServerSideProps(context) {
  ...
  const sqlQuery =
    "SELECT blog_id, blog_title, blog, description FROM blogs WHERE blog_id = ?";
